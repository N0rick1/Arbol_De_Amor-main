...
function showSignature() {
  ...
  signature.textContent = firma ? decodeURIComponent(firma) : "Con amor, Alvaro <3";
...
function showCountdown() {
  const container = document.getElementById('countdown');
    ...
    let startDate = new Date('2025-07-09T00:00:00'); // 9 de julio 2025

    // 📅 Fecha del primer mes (9 de agosto 2025)
    let today = new Date();
    let anniversaryYear = today.getFullYear();
    let eventDate = new Date(anniversaryYear, 7, 9); // Agosto = 7

    // Si el aniversario ya pasó este año, poner para el próximo año
    if (eventDate < today) {
        eventDate.setFullYear(anniversaryYear + 1);
    }
    ...
    function update() {
        ...
        // Calcular días para el aniversario
        ...
        container.innerHTML =
            ...
            `Nuestro aniversario de primer es: <b>${eventDays > 0 ? 'dentro de ' + eventDays + ' días' : '¡Es hoy!'}</b>`;
